...
(function () {
  ...
  // Shaders (kept nearly same as original)
  ...
  // Core classes adapted from the React version — rendering directly with the WebGLRenderer
  class App {
    constructor(container, options = {}) {
      this.options = Object.assign({}, effectOptionsDefaults, options);
      this.container = container;

      if (!this.container) {
        console.warn('Hyperspeed: Container not found. Effect disabled.');
        this.disposed = true;
        return;
      }
      ...
      this.camera = new THREE.PerspectiveCamera(
        this.options.fov,
        (container && container.offsetWidth) / (container && container.offsetHeight),
        0.1,
        10000
      );
    ...
    update(delta) {
      ...
      if (this.options.distortion && this.options.distortion.getJS) {
        const distortion = this.options.distortion.getJS ? this.options.distortion.getJS(0.025, time) : new THREE.Vector3(0, 0, 0);
  ...
  // CarLights, LightsSticks, Road classes: mostly unchanged except they rely on app.renderer render path.
  class CarLights {
    ...
    init() {
      ...
      let material = new THREE.ShaderMaterial({
        ...
        transparent: true,
        uniforms: Object.assign({ uTime: { value: 0 }, uTravelLength: { value: options.length }, uFade: { value: this.fade } }, this.webgl.fogUniforms, this.webgl.options ? this.webgl.options.distortion.uniforms : {})
      });

      material.onBeforeCompile = shader => {
        shader.vertexShader = shader.vertexShader.replace('#include <getDistortion_vertex>', this.webgl.options.distortion.getDistortion);
      };
    ...
    update(time) { if (this.mesh && this.mesh.material && this.mesh.material.uniforms) this.mesh.material.uniforms.uTime.value = time; }
  }
  ...
  class LightsSticks {
    ...
    init() {
      ...
      const material = new THREE.ShaderMaterial({
        ...
        uniforms: Object.assign({ uTravelLength: { value: options.length }, uTime: { value: 0 } }, this.webgl.fogUniforms, this.webgl.options.distortion.uniforms)
      });
  ...
  class Road {
    ...
    createPlane(side, width, isRoad) {
      ...
      const material = new THREE.ShaderMaterial({ fragmentShader: isRoad ? roadFragment : islandFragment, vertexShader: roadVertex, side: THREE.DoubleSide, uniforms: Object.assign(uniforms, this.webgl.fogUniforms, this.webgl.options.distortion.uniforms) });
  ...
  function resizeRendererToDisplaySize(renderer, setSize) {
    ...
    if (needResize) setSize(width, height);
    return needResize;
  }

  // Init: create container if missing and mount app
  document.addEventListener('DOMContentLoaded', () => {
    const containerId = 'lights';
    let container = document.getElementById(containerId);
    if (!container) {
      return;
    }

    const options = Object.assign({}, effectOptionsDefaults);
    options.distortion = distortions[options.distortion];

    const app = new App(container, options);
    // attach options to webgl for easier access in classes
    app.options = options;
    // make options available to submodules
    app.options.distortion = options.distortion;

    // ensure the distortion uniforms are accessible through app.options
    app.init();
  });
